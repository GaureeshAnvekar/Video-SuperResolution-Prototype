...
def decode(decode_queue, encode_queue, data_queue, shared_tensor_list):
    while True:
        try:
            ...
            #header_file = input[0]
            video_file = input[0] #this 240p mp4 video
            output_input = input[1]
            video_info = input[2]
            segment_number = input[3]
            ...
            #basename gives last part i.e. video.mp4
            #splitext gives a pair (path from root till file, extension of file)
            video_file_name, _  = os.path.splitext(os.path.basename(video_file))
            process_dir = os.path.join(opt.result_dir, '240pUpscaled')
            os.makedirs(process_dir, exist_ok=True)

	    # Avoid this step as we already have an mp4 file. So no input file will be saved in the directory made up above.
	    #1. merge 'Header.m4s' and 'X.m4s' for decoding
            ...
            target_height = t_h
            encode_queue.put(('start', process_dir, video_info,  segment_number))
            encode_queue.join()
            ...
            while True:
                ...
                data_queue.put(('apply_sr', frame_count, output_input))
                frame_count += 1
            vc.release()
        ...
        try:
            ...
            #apply super-resolution
                ...
                with torch.no_grad():
                    ...
                    encode_queue.put(('encode', frame_count % SHARED_QUEUE_LEN, output_input))
        ...
        try:
            ...
            if input[0] == 'start':
                ...
                command = [ 'ffmpeg',
                            ...
                            '{}'.format(os.path.join(process_dir, 'output_{}.mp4'.format(segment_number)))]
                """
                command = [ 'ffmpeg',
                            '-r', str(fps), # frames per second
                            '-y',
                ...
                pipe = sub.Popen(command, stdin=sub.PIPE, stderr=sub.PIPE, stdout=sub.PIPE, bufsize=0)
                end_time_ = time.time()
                ...
                #print(idx)
                def enqueue_output(outp, segmentBytesQ, output_input):
                    #f = open('/home/gaureeshanvekar/Video-SuperResolution-Prototype/pythonUpscaleScripts/testoutput.mp4', 'ab')
                    #f.write(outp.read())
                    #f.close()
                    #segmentBytesQ.put("NEXT PART")
                    #print(outp.read(), flush=True)
                    #segmentBytesQ.put(outp.read())  #this worked
                    output_input.send(outp.read())
                    ...
                    #with open("/home/gaureeshanvekar/Video-SuperResolution-Prototype/pythonUpscaleScripts/testoutput.mp4", "wb+") as sys.stdout:
                    # print(outp.read())
                ...
                if (idx == 0):
                    #print("thread called")
                    th = threading.Thread(target=enqueue_output, args=(pipe.stdout, segmentBytesQ, output_input))
                    th.daemon = True
                    th.start()

                #pipe.stdin.write( base64.b64encode(img.tobytes(), 'strict'))
                pipe.stdin.write(img.tobytes())
                #pipe.stdout.readline()      
                pipe.stdin.flush()
                pipe.stdout.flush()
                #pipe.communicate(input=img.tobytes())
                #pipe.communicate()[0]
            ...
            #save as a video
                ...
                start_time_ = time.time()
                output_input = input[1]
                #output_input.send(bytes('end', 'utf-8'))
                #pipe.stdin.flush()
                #pipe.stdout.read()
                pipe.stdin.close()
                #pipe = None
                ...
                #output_input.send(('output'))
                #output_input.send(segmentBytesQ.get())
                #output_input.send(bytes('end', 'utf-8'))
                process_dir = None
                #print("DONE")
...
#building a request
def request(decode_queue, resolution, video, index, fps=24.0, duration=4.0, sendVideoConn=None):
    res2quality = {240: 0, 360: 1, 480: 2, 720: 3, 1080: 4}
    #video_dir = os.path.join(opt.data_dir, '{}p'.format(resolution))
    data_name = opt.data_name    
    video_dir = os.path.join("./videos/" + video, "{}p".format(resolution))
    start_time = time.time()
    ...
    #Only 3 paramters set, as excluding init.mp4 and passing seg.mp4 instead of .m4s
   
    decode_queue.put((os.path.join(video_dir, video + '_240p_segment_{}.mp4'.format(index)), output_input, video_info, index))

    while(1):
        #input = output_output.recv()
       
        segmentBytes = output_output.recv()
        sendVideoConn.send(segmentBytes)
        #sendVideoConn.send(bytes('end', 'utf-8'))
        break
        #print(segmentBytes)
        """
        if video[0] == 'output':
