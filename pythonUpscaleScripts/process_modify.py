    ...
    while(1):
        try:
            ...
            if input[0] == 'start':
                ...
                if (idx == 0):
                    print("thread called")
                    th = threading.Thread(target=enqueue_output, args=(pipe.stdout, segmentBytesQ))
                    th.daemon = True
                    th.start()
...
#building a request
def request(decode_queue, resolution, index, fps=24.0, duration=4.0, sendVideoConn=None):
    ...
    print(video_dir);
    decode_queue.put((os.path.join(video_dir, 'news_240p_segment_{}.mp4'.format(index)), output_input, video_info, index))
